...
var (
	...
	magellanLog   *rotatelogs.RotateLogs
...
func init() {
	...
	// default magellan log dir setting
	if !strings.HasPrefix(logdir, "/") {
		if logdir = os.Getenv("APP_PATH"); logdir == "" {
			logdir = os.Getenv("GOPATH") + "/src/magellan"
		}
		logdir = logdir + "/log"
	}
	...
	// Set magellan logging.
	magellanLog, _ = rotatelogs.New(
		logdir+"/magellan_log.%Y%m%d%H%M",
		rotatelogs.WithLinkName(logdir+"/magellan_log"),
		rotatelogs.WithMaxAge(time.Duration(logMaxAge)*24*time.Hour),
		rotatelogs.WithRotationTime(time.Hour),
	)
	...
	Logger = zerolog.New(magellanLog).With().Timestamp().Logger()
}
